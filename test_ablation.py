...
from model.CAFFUNet import CAFFUNet
...
def test(image_array, net):
    ...
    with torch.no_grad():
        ...
        preds_p, preds_v = net(images)

        # sigmoid将一个real value映射到（0, 1）的区间，用来做二分类
        preds_p = torch.sigmoid(preds_p)  # 值域0-1之间
        preds_p = (preds_p >= 0.5).float()  # (1, 1, 192, 256)
        preds_v = torch.sigmoid(preds_v)  # 值域0-1之间
        preds_v = (preds_v >= 0.5).float()  # (1, 1, 192, 256)
    ...
    return preds_p, preds_v


if __name__ == '__main__':
    args = configs.args
    coarse_weights = 'experiments/Dual_UNet/best_model.pth'  # 权重地址
    ...
    for i, name in tqdm(enumerate(test_loader), total=len(test_loader)):
        ...
        # 预测斑块和血管
        plaque_predict_array, vessel_predict_array = test(resize_image_arr, net)  # (1, 192, 256)
    ...
    print("Vessel Test Patient Avg Dice：{:.2%}  IoU：{:.2%} sen：{:.2%} pre：{:.2%}".
          format(v_dice / len(v_dice_dict), v_iou / len(v_iou_dict), v_sen / len(v_sen_dict), v_pre / len(v_pre_dict)))
    ...
    logging.info('Vessel——Testing performance in best val model: mean_dice : %f mean_iou : %f mean_sen : %f mean_pre : %f'
                 % (v_dice / len(v_dice_dict), v_iou / len(v_iou_dict), v_sen / len(v_sen_dict), v_pre / len(v_pre_dict)))
