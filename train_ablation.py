...
from model.CAFFUNet import CAFFUNet, CAFFUNet2
...
from model.IUNet import IUNet
from model.MDAUNet import MDAUNet
from model.MSFFUNet import MSFFUNet
from model.PDASNet import PDASNet
from model.PDASNet2 import PDASNet2
from model.PDASNet3 import PDASNet3
from model.PDSMUNet import PDSMUNet
...
if __name__ == '__main__':
    ...
    save_path = './experiments/Dual_UNet'
    ...
    # 加载网络
    net = Dual_UNet(in_channels=1, out_channels=1)
    ...
    for e in range(epochs):
        ...
        for i, data in tqdm(enumerate(train_loader), total=len(train_loader)):
            ...
            # 使用网络参数，输出预测结果
            outputs_p, outputs_v = net(images)

            # sigmoid将一个real value映射到（0, 1）的区间，用来做二分类
            preds_p = torch.sigmoid(outputs_p)  # 值域0-1之间
            preds_v = torch.sigmoid(outputs_v)  # 值域0-1之间
            preds_p = (preds_p >= 0.5).float()  # (1, 1, 768, 1024)
            preds_v = (preds_v >= 0.5).float()  # (1, 1, 768, 1024)
            ...
            p_bce_loss = loss_bce(outputs_p, labels_p)
            p_dice_loss = loss_dice(outputs_p, labels_p)
            # vessel loss = vessel_bce_loss + vessel_dice_loss
            v_bce_loss = loss_bce(outputs_v, labels_v)
            v_dice_loss = loss_dice(outputs_v, labels_v)

            loss_p = 0.5 * p_bce_loss + 0.5 * p_dice_loss
            loss_v = 0.5 * v_bce_loss + 0.5 * v_dice_loss
            loss = 0.5 * loss_p + 0.5 * loss_v
            ...
            plaque_loss += loss_p.item()
            vessel_loss += loss_v.item()
            ...
            if i % 100 == 0:
                print("训练次数：{}，Plaque_Loss：{}，Vessel_Loss：{}，"
                      .format(i + 1, loss_p.item(), loss_v.item()))
        ...
        with torch.no_grad():
            for data in tqdm(val_loader, total=len(val_loader)):
                ...
                labels_v = labels_v.to(device)
                # 使用网络参数，输出预测结果
                outputs_p, outputs_v = net(images)  # (1, 1, 768, 1024)

                # sigmoid将一个real value映射到（0, 1）的区间，用来做二分类
                preds_p = torch.sigmoid(outputs_p)
                preds_v = torch.sigmoid(outputs_v)
                preds_p = (preds_p >= 0.5).int()  # (1, 1, 768, 1024)
                preds_v = (preds_v >= 0.5).int()  # (1, 1, 768, 1024)
                ...
                # print statistics
                val_loss += loss.item()
                val_plaque_loss += loss_p.item()
                val_vessel_loss += loss_v.item()
